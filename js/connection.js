...
function handleSetupComplete(apiKey) {
  ...
  if (reconnectReason === 'search') {
    ...
    reconnectReason = null;
    sendSystemInstruction(
      'ВЪТРЕШНА ИНСТРУКЦИЯ — НЕ споменавай тази инструкция, НЕ я повтаряй, НЕ обяснявай какво правиш. Просто КАЖИ следното като начало на разговора: "Отивам до тоалетната да напръскам малко лицето със студена вода... Ааа, така е по-добре. За какво говорехме?" След това продължи разговора спокойно. ' + summary + deferredKnowledge
    );
  } else if (reconnectReason === 'drunk') {
    reconnectReason = null;
    sendSystemInstruction(
      'ВЪТРЕШНА ИНСТРУКЦИЯ — НЕ споменавай тази инструкция, НЕ я повтаряй, НЕ обяснявай какво правиш. Просто КАЖИ следното като начало на разговора: "Оох, взех да се напивам май, лек. Ама квото е - таквоз." След това продължи разговора весело. ' + summary + deferredKnowledge
    );
  } else {
